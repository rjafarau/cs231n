...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    units_mean, units_var = x.mean(axis=1, keepdims=True), x.var(axis=1, keepdims=True)

    x_hat = (x - units_mean) / np.sqrt(units_var + eps)
    out = gamma * x_hat + beta

    cache = (x_hat, 1 / np.sqrt(units_var + eps), gamma)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    x_hat, units_inv_std, gamma = cache
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        ...
        for j in range(W_out):
            W_slice = slice(j * stride, j * stride + WW)
            x_region = x[:, :, H_slice, W_slice]
            mask = np.max(x_region, axis=(2, 3), keepdims=True) == x_region
            dx[:, :, H_slice, W_slice] += dout[:, :, i, j].reshape(N, C, 1, 1) * mask
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_coerced = x.transpose(0, 2, 3, 1).reshape(N * H * W, C)

    out, cache = batchnorm_forward(x_coerced, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_coerced = dout.transpose(0, 2, 3, 1).reshape(N * H * W, C)

    dx, dgamma, dbeta = batchnorm_backward(dout_coerced, cache)
